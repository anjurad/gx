...
2. Create the metrics table using `sql/create_dq_metrics.sql`.
...
`local_mvp/data/green_tripdata_2017_sample.csv`.
...
uv venv --python /usr/bin/python3.13 .venv313

# 3) Install project dependencies
uv pip install --python .venv313/bin/python -e .

# 4) Run local GX validation via the wrapper API
.venv313/bin/python src/optimised-04.py
```
